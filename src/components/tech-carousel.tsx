        ...
        <div className="flex justify-center items-center gap-8 flex-wrap">
          {technologies.map((tech, index) => (
            <motion.div
              key={tech.name}
              className={cn(
                "relative flex flex-col items-center transition-all duration-300 px-4 py-2",
            ...
            >
              ...
              {activeIndex === index && (
                <motion.div className="absolute -bottom-1 left-0 right-0 h-0.5 bg-accent" layoutId="activeIndicator" />
              )}
            </motion.div>
          ))}
        </div>
